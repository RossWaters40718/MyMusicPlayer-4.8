...
class Audio_Player():
    ...
    def play(self,key=None):
        ...
        if _stopped:self.active_song=""
            ...
            if self.key_now==len(list(self.Songs))-1:
                key=0
                self.select_song(0)
            else:self.select_song(self.key_now+1)
        elif _previous:
            if self.key_now==0:
                key=len(list(self.Songs))-1
    ...
    def position_art(self):
        ...
        try:
            ...
            if root.state()=='normal':
                ...
                if self._x==int(orig_x):# Startup Center Screen
    ...
    def on_resize(self,event):
        if event.widget is Song_Scroll or event.widget is Song_Scroll.vbar or event.widget is Song_Scroll.hbar:
            return
        ...
        if root.state()=='zoomed':
            ...
            button_font['size']=9
            if np.all(self.art_size==(0.0,0.0)):# No Art
                self._wid,self._hgt=root.winfo_width(),root.winfo_height()
                self._x,self._y=root.winfo_x(),root.winfo_y()
        self.position_art()        
...
if __name__ == '__main__':
    ...
    root.bind("<B1-Motion>", Player.on_resize)
